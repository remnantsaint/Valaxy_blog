...
#### K8s
  全称 Kubernetes，管理容器的部署、扩缩容和故障恢复。

  当 Docker 服务变多、服务器变多，就会出现以下问题：
...
| 概念 | 通俗理解 |
| ---- | ---- |
| Cluster（集群） | 一组由 K8s 统一管理的机器整体 |
| Node（节点） | 集群里的单台机器，可以是物理机或者虚拟机 |
| Pod | K8s最小部署单元，里面包含一个或多个紧密配合的容器 |
| Deployment | 声明应用需要运行多少副本，负责版本更新、故障补副本 |
| Service | 给一组Pod提供固定不变的访问入口 |

  例如，声明“预测服务运行两份”，实际只剩一份时，Deployment 对应的控制机制会尝试补足副本。Pod 地址可能变化，其他服务通过 Service 访问，就不必一直追踪具体 Pod 地址。

  Docker 提供镜像构建和容器运行等能力，K8s 侧重在集群中编排和管理容器。它们解决的问题层次不同，K8s 也不要求必须使用 Docker Engine 作为运行时（负责真正启动、停止容器的底层程序）。
