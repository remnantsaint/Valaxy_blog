...
### Redis
  Redis 是以内存存取为主的数据存储系统，支持多种数据结构。可以把它理解成一个独立运行、程序通过网络访问的“高速数据柜”。
...
# 设置 code = 123456，60 秒后自动过期删除
SET code "123456" EX 60

# 查看 code 剩余还有多少秒过期
TTL code

# 对 visits 的值 +1（原子自增，常用于统计计数）
INCR visits

# 哈希类型：在 user:1 这个哈希里，设置 name 和 age 两个字段
HSET user:1 name "Alice" age 23

# 哈希类型：读取 user:1 哈希中的 name 字段的值
HGET user:1 name
```
  缓存应该包含一些经常取用的信息。

  Redis 为什么快？
    * 主要是因为数据在内存中处理、数据结构适合对应操作。实际速度也受网络、命令复杂度和数据大小影响，不能说所有操作都很快。

  Redis 与 PostgreSQL / MySQL 有什么区别？
    * 关系型数据库擅长关系建模、SQL 查询、约束和事务；Redis 常用于缓存、计数等场景。两者通常配合使用

  Redis 重启后数据一定丢失吗？
    * 不一定，可以配置持久化。RDB 保存某一时刻的数据快照，AOF 记录写操作；能恢复到什么程度取决于配置和故障情况。

  过期和内存淘汰有什么区别？
    * 过期是键达到有效期；内存淘汰是在达到配置的内存限制时，按策略移除数据。并非所有淘汰策略都会主动删除旧键，也可能拒绝新写入。

  缓存穿透：反复查询根本不存在的数据，每次都查到数据库，压垮 DB。应该参数校验进行拦截；短期缓存空结果，直接返回不再访问数据库。

  缓存击穿：某个热点键过期，大量请求同时计算或查库。添加互斥锁，避免并发查库；热点键设置永不过期。

  缓存雪崩：大量缓存同时失效，或缓存服务故障。应该过期时间打散，避免集体失效；限流、熔断、降级，Redis 挂了直接返回兜底，保护数据库。
